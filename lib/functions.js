...
export const getAllPaymentRecord = async (userId) => {
  document.querySelector('#generalLoading').classList.remove('hidden')
  document.querySelector('#generalLoading').classList.add('grid')
  if (userId) {
    try {
      const response = await fetch('/api/user', {
        method: 'POST',
        body: JSON.stringify(['getAllPaymentRecord', userId]),
        type: 'application/json'
      })
      if (response.status == 200) {
        const res = await response.json()
...
// pervalidrefer.js
export const getAllValidRefer = async (userId) => {
  document.querySelector('#generalLoading').classList.remove('hidden')
  document.querySelector('#generalLoading').classList.add('grid')
  if (userId) {
    try {
      const response = await fetch('/api/rfcommission', {
        method: 'POST',
        body: JSON.stringify(['getAllValidRefer', userId]),
        type: 'application/json'
      })
      if (response.status == 200) {
        const res = await response.json()
...
// investmentreturn.js
export const getAllIrc = async (userId) => {
  document.querySelector('#generalLoading').classList.remove('hidden')
  document.querySelector('#generalLoading').classList.add('grid')
  if (userId) {
    try {
      const response = await fetch('/api/rfcommission', {
        method: 'POST',
        body: JSON.stringify(['getAllIrc', userId]),
        type: 'application/json'
      })
      if (response.status == 200) {
        const res = await response.json()
